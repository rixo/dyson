...
var generate = function(req, res, next) {
    ...
    var service = this,
        // @todo rixo test bintToTemplate
        bindToTemplate = service.bindToTemplate !== false
            && (service.bindToTemplate || util.options.get('bindToTemplate')),
        exposeRequest = service.exposeRequest !== false
            && (service.exposeRequest || util.options.get('exposeRequest')),
        path = req.url,
        overrides = req.overrides,
        status = _.isUndefined(req.overriddenStatus) ? service.status : req.overriddenStatus;
    ...
    // @todo rixo test overridden scalar status
    if (status != null) {
        if(_.isFunction(status)) {
            status.call(service, req, res);
        } else {
            res.status(status);
        }
    }

    function setValuesWithResolved() {
        if (overridePromise) {
      ...
      function done() {
          if (bindToTemplate) {
              return setValues(template, templateArgs, template);
          } else {
        ...
        _.forEach(template, function(value, key) {
            if(template.hasOwnProperty(key)) {
                obj[key] = _.isFunction(value)
                    ...
                    : _.isPlainObject(value) ? setValues(value, params, scope || obj) : value;
